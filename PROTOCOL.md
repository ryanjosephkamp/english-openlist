...
| **SR2** | **Halt if the corpus arm does not break the all-zero stratum.** After full ingest, the **undifferentiated stratum** — words with an all-zero binary detector pattern AND no corpus data — may not exceed **50%** of the frame (reading B, operative per D-032). | Above that, the majority of the population shares one posterior and per-word probabilities are not a meaningful output. Measured at Phase 1 close: **8.63%** (845,150 of 9,787,841) — passed. The binary-pattern reading (A) is reported as a diagnostic of S1–S3's reach: **90.50%**. |
